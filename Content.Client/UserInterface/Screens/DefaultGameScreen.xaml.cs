...
public sealed partial class DefaultGameScreen : InGameScreen
{
    ...
    // Clears whichever bottom-band module is taller. The action bar is centre-left and the alerts
    // grow rightward from the edge, so at narrow widths they meet.
    private const float BandTallest =
        VitalsTallest > ActionsBar.BarHeight ? VitalsTallest : ActionsBar.BarHeight;

    private const float AlertsLift = BottomBandLift + BandTallest + 6f;
