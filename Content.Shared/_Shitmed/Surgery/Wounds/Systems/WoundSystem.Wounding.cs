...
public sealed partial class WoundSystem
{
    ...
    private void OnWoundInserted(EntityUid uid, WoundComponent comp, EntGotInsertedIntoContainerMessage args)
    {
        ...
        var bodyPart = Comp<OrganComponent>(comp.HoldingWoundable);
    ...
    private void OnWoundableInserted(EntityUid parentEntity, WoundableComponent parentWoundable, EntInsertedIntoContainerMessage args)
    {
        ...
        if (TryComp<OrganComponent>(parentEntity, out var bodyPart)
    ...
    private void DudeItsJustLikeMatrix(EntityUid uid, WoundableComponent comp, ref BeforeDamageChangedEvent args)
    {
        ...
        var bodyPart = Comp<OrganComponent>(uid);
        if (args.Origin != null)
        {
    ...
    private void OnWoundSeverityPointChanged(EntityUid uid, WoundComponent component, WoundSeverityPointChangedEvent args)
    {
        ...
        if (TerminatingOrDeleted(uid)
            ...
            || !TryComp<OrganComponent>(component.HoldingWoundable, out var bodyPart))
            return;
    ...
    private void OnAttemptHandsMelee(EntityUid uid, WoundableComponent component, ref AttemptHandsMeleeEvent args)
    {
        if (component.WoundableIntegrity > 25
            || args.Handled
            || !TryComp(uid, out OrganComponent? bodyPart)
        ...
        WoundableComponent? woundable = null)
    {
        ...
        var bodyPart = Comp<OrganComponent>(wound.HoldingWoundable);
        ...
        WoundableComponent? woundable = null)
    {
        if (!Resolve(uid, ref wound)
            || !Resolve(wound.HoldingWoundable, ref woundable)
            || !TryComp(wound.HoldingWoundable, out OrganComponent? bodyPart))
            return;
        ...
        var upperLimit = wound.WoundSeverityPoint + woundable.WoundableIntegrity.Value;
    ...
    {
        if (!TryComp<OrganComponent>(woundableEntity, out var bodyPart))
            return;
    ...
    public void AmputateWoundable(EntityUid parentWoundableEntity, EntityUid woundableEntity, WoundableComponent? woundableComp = null)
    {
        ...
        if (woundableComp.DamageOnAmputate != null
            && _lookup.TryGetRootOrgan(bodyPart.Body.Value, out var rootPart)
        ...
        BodyComponent? bodyComp = null)
    {
        ...
        foreach (var woundEnt in GetWoundableWounds(parent, woundableComp))
        {
            ...
            tourniquetable.SeveredSymmetry = bodyPart.Category;
            tourniquetable.SeveredPartType = bodyPart.Category;
        }*/
        // Goobstation end
    }
        ...
        WoundableComponent childWoundable)
    {
        ...
        foreach (var (woundId, wound) in GetAllWounds(childEntity, childWoundable))
        {
            ...
            var bodyPart = Comp<OrganComponent>(childEntity);
            if (bodyPart.Body.HasValue)
            {
    ...
    private void CheckWoundableSeverityThresholds(EntityUid woundable, WoundableComponent? component = null)
    {
        ...
        var bodyPart = Comp<OrganComponent>(woundable);
        if (bodyPart.Body == null)
            return;
        ...
        bool ignoreBlockers = false)
    {
        ...
        if (healable)
        {
            return GetWoundableWounds(targetEntity, targetWoundable)
                .Where(wound => _prototype.Index(wound.Comp.DamageGroup)?.ID == damageGroup || damageGroup == null)
                .Where(wound => CanHealWound(wound, wound.Comp, ignoreBlockers))
                .Aggregate(FixedPoint2.Zero, (current, wound) => current + (FixedPoint2) wound.Comp.WoundIntegrityDamage);
        }
