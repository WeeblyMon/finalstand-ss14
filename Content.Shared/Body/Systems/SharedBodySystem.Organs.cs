...
public partial class SharedBodySystem
{
    ...
    /// <summary>
    /// Compatibility alias for vanilla callers that expect Entity&lt;T&gt; (without OrganComponent).
    /// Always returns a non-null list (empty when body has no matching organs).
    /// </summary>
    public bool TryGetOrgansWithComponent<T>(
        EntityUid body,
        out List<Entity<T>> organs,
        BodyComponent? bodyComp = null)
        where T : IComponent
    {
        if (!TryGetBodyOrganEntityComps<T>((body, bodyComp), out var found))
        {
            organs = new List<Entity<T>>();
            return false;
        }
        organs = found.ConvertAll(e => new Entity<T>(e.Owner, e.Comp1));
        return true;
    }
