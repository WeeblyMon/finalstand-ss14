...
public partial class SharedBodySystem
{
    ...
    protected const float GibletLaunchImpulse = 8;
    protected const float GibletLaunchImpulseVariance = 3;
    ...
    private void MapInitBody(EntityUid bodyEntity, BodyPrototype prototype)
    {
        ...
        MapInitParts(bodyEntity, rootPartUid, rootPart, prototype);
    }
    ...
    private void MapInitParts(EntityUid body, EntityUid rootPartId, BodyPartComponent rootPart, BodyPrototype prototype)
    {
        ...
        List<string>? excludedContainers = null)
    {
        ...
        if (TryGetRootPart(bodyId, out var root) && TryComp(root.Value.Owner, out GibbableComponent? gibbable))
        ...
        {
            // Detach the part from its container before the body entity is deleted,
            // otherwise container cleanup will delete the part too.
            SharedTransform.AttachToGridOrMap(part.Id);
            _gibbingSystem.Gib(part.Id, dropGiblets: true);
            gibs.Add(part.Id);
        ...
        SoundSpecifier? gibSoundOverride = null)
    {
        ...
        var partGibs = _gibbingSystem.Gib(partId, dropGiblets: true);
        foreach (var g in partGibs)
            gibs.Add(g);
        gibs.Add(partId);
            ...
            && bodyPart is not null)
        {
                ...
                && !GetBodyChildrenOfType(args.EquipTarget, bodyPart.Value).Any())
            {
                _popup.PopupClient(Loc.GetString("equip-part-missing-error",
                    ("target", args.EquipTarget), ("part", bodyPartString)), args.User, args.User);
                args.Cancel();
            }
        }
    }
