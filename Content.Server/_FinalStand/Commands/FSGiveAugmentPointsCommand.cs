...
public sealed class FSGiveAugmentPointsCommand : IConsoleCommand
{
    [Dependency] private readonly ISharedPlayerManager _playerManager = default!;

    public string Command => "fsgiveap";
    public string Description => "Give (or take) augment points from yourself or another player.";
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        IoCManager.Resolve<IEntityManager>().System<FSPlayerWalletSystem>().GiveAugmentPoints(session, amount);

        var verb = amount >= 0 ? $"Gave +{amount}" : $"Took {amount}";
        shell.WriteLine($"{verb} augment points to {session.Name}.");
    }
}
