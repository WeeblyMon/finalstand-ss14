...
namespace Content.Client.Chemistry.UI
{
    ...
    public sealed partial class ChemMasterWindow : FancyWindow
    {
        ...
        {
            var held = new Dictionary<string, FixedPoint2>();
            foreach (var (reagent, quantity) in state.BufferReagents)
                held[reagent.Prototype] = quantity;

            var key = string.Join('|', held.Keys.OrderBy(x => x));
            if (key == _recipeKey)
                return;
            ...
            if (held.Count == 0)
            {
                ...
                {
                    Text = Loc.GetString("chem-master-window-recipes-empty"),
            ...
            foreach (var reaction in _prototypeManager.EnumeratePrototypes<ReactionPrototype>())
            {
                ...
                foreach (var (reactantId, reactant) in reaction.Reactants)
                {
                    ...
                    _prototypeManager.TryIndex(reactantId, out ReagentPrototype? missingProto);
                    missing = missingProto?.LocalizedName ?? reactantId;
                }

                if (matched == reaction.Reactants.Count)
                    ready.Add(GetReactionName(reaction));
                else if (missing != null && matched > 0)
                    nearMisses.Add(Loc.GetString("chem-master-window-recipe-needs", ("recipe", GetReactionName(reaction)), ("reagent", missing)));
            }
            ...
            if (ready.Count == 0 && nearMisses.Count == 0)
            {
                RecipeInfo.AddChild(new Label
                {
                    Text = Loc.GetString("chem-master-window-recipes-none"),
                    StyleClasses = { StyleClass.LabelWeak },
                });
                return;
            }
            ...
            if (ready.Count > 0)
            {
                RecipeInfo.AddChild(new Label
                {
                    Text = Loc.GetString("chem-master-window-recipes-ready", ("recipes", string.Join(", ", ready.OrderBy(x => x).Take(8)))),
                    Modulate = Color.FromHex("#4FBF7A"),
                });
            }

            foreach (var miss in nearMisses.OrderBy(x => x).Take(6))
            {
                RecipeInfo.AddChild(new Label
                {
                    Text = miss,
        ...
        private string GetReactionName(ReactionPrototype reaction)
        {
            ...
            {
                if (_prototypeManager.TryIndex(product, out ReagentPrototype? proto))
                    return proto.LocalizedName;
            }
